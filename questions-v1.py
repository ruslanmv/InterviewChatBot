...
def generate_questions_from_text(text, n_questions=5):
    ...
    # LangChain's ChatOpenAI wrapper
    chat = ChatOpenAI(
        openai_api_key=openai_api_key, model="gpt-4", temperature=0.7, max_tokens=750
    )

    # System and user messages for structured conversation
    messages = [
        SystemMessage(
    ...
    try:
        ...
        # Generate response using `invoke` (to avoid deprecated `__call__`)
...
def generate_and_save_questions_from_pdf(pdf_path):
    ...
    for i, chunk in enumerate(chunks):
        ...
        # Generate default 5 questions per chunk
        questions = generate_questions_from_text(chunk, n_questions=5)
...
if __name__ == "__main__":
    ...
    try:
        generated_questions = generate_and_save_questions_from_pdf(pdf_path)
