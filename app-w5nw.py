...
def load_questions():
    if os.path.exists(QUESTIONS_PATH):
        with open(QUESTIONS_PATH, "r") as f:
            ...
            if questions:
                print(f"[DEBUG] Loaded {len(questions)} technical questions.")
            else:
                print("[DEBUG] No questions found in questions.json.")
            return questions
    else:
        print("[DEBUG] questions.json not found.")
        return []
...
def reset_interview_action_old(voice):
    ...
    if config["type_of_interview"] == "Technical":
        ...
        if not interview_state.technical_questions:
            print("[ERROR] No technical questions loaded.")
            return (
                [{"role": "assistant", "content": "No technical questions available. Please contact the admin."}],
                None,
...
def reset_interview_action_old2(voice):
    interview_state.reset(voice)
    config = interview_state.config
    n_of_questions = config.get("n_of_questions", 5)
    print(f"[DEBUG] Interview reset. Voice: {voice}")

    # Check if the interview type is "Technical"
    if config["type_of_interview"] == "Technical":
        interview_state.technical_questions = load_questions()
        if not interview_state.technical_questions:
            print("[ERROR] No technical questions loaded.")
        ...
        # Get the first technical question
        first_question = interview_state.technical_questions[0]
        print(f"[DEBUG] First Technical question loaded: {first_question}")

        # Construct the initial technical question prompt
        initial_message = {
            "role": "assistant",
            "content": get_interview_prompt_technical(language, n_of_questions, first_question)
        }
    else:
        # For Standard interviews, load the initial HR introduction
        initial_message = {
            "role": "assistant",
            "content": get_interview_initial_message_hr(n_of_questions)
        }
    ...
    # Return the initial message and audio output with interactive textbox for user response
    return (
        [initial_message],
        gr.Audio(value=temp_audio_path, autoplay=True),
...
def reset_interview_action(voice):
    ...
    # Unified introduction message for both types
    initial_message = {
        "role": "assistant",
        "content": get_interview_initial_message_hr(n_of_questions)
    }
    ...
    # After the introduction, switch to questions based on the interview type
    if config["type_of_interview"] == "Technical":
        interview_state.technical_questions = load_questions()
        ...
        # Prepare the first technical question
        first_question = interview_state.technical_questions[0]
        print(f"[DEBUG] First Technical question loaded: {first_question}")
        ...
        first_audio_buffer = BytesIO()
        convert_text_to_speech(first_question_message["content"], first_audio_buffer, voice)
        first_audio_buffer.seek(0)

        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as temp_file:
            first_audio_path = temp_file.name
            temp_file.write(first_audio_buffer.getvalue())

        interview_state.temp_audio_files.append(first_audio_path)
        print(f"[DEBUG] Audio file saved at {first_audio_path}")

        return (
            [initial_message, first_question_message],
            gr.Audio(value=first_audio_path, autoplay=True),
            gr.Textbox(interactive=True, placeholder="Type your answer here...")
        )
    else:
        # Standard interview follows default HR questions
        return (
            [initial_message],
            gr.Audio(value=temp_audio_path, autoplay=True),
            gr.Textbox(interactive=True, placeholder="Type your answer here...")
...
def bot_response_old(chatbot, message):
    ...
    question_number = interview_state.question_count
    ...
    if config["type_of_interview"] == "Standard" or not interview_state.knowledge_retrieval_setup:
        ...
        print(f"[DEBUG] Sending Standard HR question #{question_number}: {response}")
    else:
        if interview_state.technical_questions:
            ...
            print(f"[DEBUG] Sending Technical question #{question_number}: {question}")
            print(f"[DEBUG] Full prompt sent to LLM: {response}")
        else:
            response = "No technical questions available. Please contact the admin."
            print("[DEBUG] No technical questions available.")
...
def bot_response(chatbot, message):
    ...
    if config["type_of_interview"] == "Standard" or not interview_state.knowledge_retrieval_setup:
        ...
        if interview_state.technical_questions:
            if question_number - 1 < len(interview_state.technical_questions):
                question = interview_state.technical_questions[question_number - 1]
                response = get_interview_prompt_technical(language, n_of_questions, question)
                print(f"[DEBUG] Sending Technical question #{question_number}: {question}")
                print(f"[DEBUG] Full prompt sent to LLM: {response}")
            else:
                response = "All technical questions have been asked. Thank you for participating."
                interview_state.interview_finished = True
                print("[DEBUG] All technical questions completed.")
        else:
            response = "No technical questions available. Please contact the admin."
            print("[DEBUG] No technical questions available.")
