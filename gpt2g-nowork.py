...
def conduct_interview(questions, language="English", history_limit=5):
    ...
    def gradio_interview(user_input, history):
        if not history:
            ...
            history.append(["AI", f"First question: {current_question}"])
            conversation_history.append({"question": current_question, "answer": ""})  # Track question with empty answer
            return history, ""

        current_question_index = len(conversation_history) - 1
        ...
        if current_question_index < len(questions):
            ...
            conversation_history[-1]["answer"] = user_input  # Update the last question with user's answer
            history.append(["User", user_input])
            history.append(["AI", response])

            if current_question_index + 1 < len(questions):
                next_question = questions[current_question_index + 1]
                conversation_history.append({"question": next_question, "answer": ""})  # Add next question placeholder
                history.append(["AI", f"Next question: {next_question}"])
            else:
                history.append(["AI", "Thank you for your time. This concludes the interview."])
...
def start_hr_chatbot():
    ...
    with gr.Blocks(css=".gradio-container { font-family: Arial, sans-serif; max-width: 700px; margin: auto; }") as demo:
        ...
        chatbot = gr.Chatbot(label="HR Chatbot")
